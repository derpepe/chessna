...
void Engine::go(int movetime)
{
	...
	if (timeExpired)
	{
		output << "info string [Engine::go] search ended: movetime expired" << std::endl;
	}
	else if (processedMoves == possibleMoves.size())
	{
		output << "info string [Engine::go] search ended: all moves evaluated" << std::endl;
	}
	else
	{
		output << "info string [Engine::go] search ended" << std::endl;
	}

	output << "info score cp " << bestScore << std::endl;
	for (const auto& move : bestMoves)
	{
		output << "info string best " << move << " score " << bestScore << std::endl;
	}
